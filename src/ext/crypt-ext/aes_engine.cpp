...
#include "aes_engine.hpp"
#include <cstring>
#include <stdexcept>

static int Pack_littleEndianToInt(const uint8_t *bs, int off)
{
    int n = bs[off] & 0xff;
    n |= (bs[++off] & 0xff) << 8;
    n |= (bs[++off] & 0xff) << 16;
...
static uint8_t Si[] = {
    ...
    246, 100, 134, 104, 152, 22,  212, 164, 92,  204, 93,  101, 182, 146, 108, 112, 72,  80,  253, 237, 185, 218,
    94,  21,  70,  87,  167, 141, 157, 132, 144, 216, 171, 0,   140, 188, 211, 10,  247, 228, 88,  5,   184, 179,
    69,  6,   208, 44,  30,  143, 202, 63,  15,  2,   193, 175, 189, 3,   1,   19,  138, 107, 58,  145, 17,  65,
    ...
    199, 49,  177, 18,  16,  89,  39,  128, 236, 95,  96,  81,  127, 169, 25,  181, 74,  13,  45,  229, 122, 159,
...
static uint32_t rcon[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a,
...
void AESEngine::init(bool forEnc, const uint8_t *key)
{
    generateWorkingKey(key, forEnc);
...
void AESEngine::generateWorkingKey(const uint8_t *key, bool forEnc)
{
    int keyLen = 16;
    if (keyLen < 16 || keyLen > 32 || (keyLen & 7) != 0)
        throw std::runtime_error("aes_engine error");

    int KC = keyLen >> 2;
    ROUNDS = KC + 6; // This is not always true for the generalized Rijndael that allows larger block sizes

    std::memset(WorkingKey, 0, sizeof(WorkingKey[0][0]) * 11 * 4);

    auto &W = WorkingKey;

    if (KC == 4)
    {
        int t0 = Pack_littleEndianToInt(key, 0);
        W[0][0] = t0;
        int t1 = Pack_littleEndianToInt(key, 4);
        W[0][1] = t1;
        int t2 = Pack_littleEndianToInt(key, 8);
        W[0][2] = t2;
        int t3 = Pack_littleEndianToInt(key, 12);
        W[0][3] = t3;
...
void AESEngine::reset()
{
}

void AESEngine::unpackBlock(const uint8_t *bytes, int off)
{
    int index = off;
...
void AESEngine::packBlock(uint8_t *bytes, int off) const
{
    int index = off;

    bytes[index++] = (uint8_t)C0;
    bytes[index++] = (uint8_t)(C0 >> 8);
    bytes[index++] = (uint8_t)(C0 >> 16);
    bytes[index++] = (uint8_t)(C0 >> 24);

    bytes[index++] = (uint8_t)C1;
    bytes[index++] = (uint8_t)(C1 >> 8);
    bytes[index++] = (uint8_t)(C1 >> 16);
    bytes[index++] = (uint8_t)(C1 >> 24);

    bytes[index++] = (uint8_t)C2;
    bytes[index++] = (uint8_t)(C2 >> 8);
    bytes[index++] = (uint8_t)(C2 >> 16);
    bytes[index++] = (uint8_t)(C2 >> 24);

    bytes[index++] = (uint8_t)C3;
    bytes[index++] = (uint8_t)(C3 >> 8);
    bytes[index++] = (uint8_t)(C3 >> 16);
    bytes[index] = (uint8_t)(C3 >> 24);
}

void AESEngine::encryptBlock(int KW[11][4])
{
    uint32_t t0 = this->C0 ^ KW[0][0];
    uint32_t t1 = this->C1 ^ KW[0][1];
    uint32_t t2 = this->C2 ^ KW[0][2];

    uint32_t r = 1, r0, r1, r2, r3 = this->C3 ^ KW[0][3];
    while (r < ROUNDS - 1)
    {
...
void AESEngine::decryptBlock(int KW[11][4])
{
    uint32_t t0 = this->C0 ^ KW[ROUNDS][0];
    uint32_t t1 = this->C1 ^ KW[ROUNDS][1];
    uint32_t t2 = this->C2 ^ KW[ROUNDS][2];

    uint32_t r = ROUNDS - 1, r0, r1, r2, r3 = this->C3 ^ KW[ROUNDS][3];
    while (r > 1)
    {
...
int AESEngine::processBlock(uint8_t *in, int inOff, int inLen, uint8_t *out, int outOff, int outLen)
{
    if ((inOff + (32 / 2)) > inLen)
        throw std::runtime_error("aes_engine error");

    if ((outOff + (32 / 2)) > outLen)
        throw std::runtime_error("aes_engine error");
    if (forEncryption)
    {
